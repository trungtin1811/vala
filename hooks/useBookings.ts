...
export function useCancelBooking() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async ({ bookingId, eventId, skillLevel, approvalStatus }: { bookingId: string; eventId: string; skillLevel: SkillLevel; approvalStatus?: 'pending' | 'approved' | 'rejected' }) => {
...
export function useToggleBookingPaid() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async ({ bookingId, isPaid }: { bookingId: string; isPaid: boolean }) => {
