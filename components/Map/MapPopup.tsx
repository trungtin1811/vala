  ...
  return (
    <div className="w-60">
      <div className="bg-gradient-to-r from-[#0052CC] to-[#0066FF] px-4 py-3">
        <h3 className="text-white font-semibold text-sm leading-snug line-clamp-2">{event.title}</h3>
        <div className="mt-1.5 flex items-center gap-2 flex-wrap">
          <EventStatusBadge status={event.status} />
          {distanceKm !== undefined && (
            <span className="text-white/70 text-xs">{formatDistance(distanceKm)}</span>
          )}
        </div>
      </div>
      <div className="px-4 py-3 flex flex-col gap-2">
        ...
        <div className="flex gap-2 pt-1">
          <Link href={`/events/${event.id}`} className="flex-1">
            <Button variant="secondary" size="sm" className="w-full text-xs py-1.5">Chi Tiết</Button>
          </Link>
          {event.status === 'active' && available > 0 && (
            <Button size="sm" className="flex-1 text-xs py-1.5" onClick={onBook}>
              Đặt Chỗ
            </Button>
          )}
        </div>
