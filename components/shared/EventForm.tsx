...
const LEVEL_GROUPS = [
  {
    label: "Mới chơi",
    levels: ["yeu_minus", "yeu", "yeu_plus"] as SkillLevel[],
  },
  {
    label: "Trung bình yếu",
    levels: ["tby_minus", "tby", "tby_plus"] as SkillLevel[],
  },
  {
    label: "Trung bình",
    levels: ["tb_minus", "tb", "tb_plus"] as SkillLevel[],
  },
  {
    label: "Trung bình khá",
    levels: ["kha_minus", "kha", "kha_plus"] as SkillLevel[],
  },
  {
    label: "Khá / Pro",
    levels: ["gioi_minus", "gioi", "gioi_plus"] as SkillLevel[],
  },
];
...
export function EventForm({
  ...
  onSubmit,
  onCancel,
  submitLabel = "Đăng bài tuyển slot",
  loading,
  hideRepeat,
}: EventFormProps) {
  const {
  ...
  function toggleGroup(levels: SkillLevel[]) {
    ...
    if (allSelected) {
      setValue(
        "skill_levels",
        current.filter((l) => !levels.includes(l)),
      );
    } else {
  ...
  function countRepeatDates(): number {
    if (!repeatEnabled || !eventDate || repeatDays.length === 0 || !repeatUntil)
      return 1;
    let count = 0;
  ...
  const cardClass =
    "bg-white border border-[#E5E7EB] rounded-2xl p-5 flex flex-col gap-4";
  const cardHeadClass =
    "flex items-center gap-2 text-[#0052CC] font-semibold text-sm";

  async function handleFormSubmit(values: EventFormValues) {
    if (!values.skill_levels || values.skill_levels.length === 0) {
      setError("skill_levels", {
        type: "manual",
        message: "Vui lòng chọn ít nhất 1 trình độ yêu cầu",
      });
      return;
    }

    clearErrors("skill_levels");
  ...
  return (
    <form
      ...
      className="flex flex-col gap-5 pb-24 sm:pb-28"
    >
                    ...
                    <span>
                      ...
                      {endInfo.isNextDay && eventDate && (
                        <span className="ml-1">
                          ...
                          {formatDisplayDate(endInfo.endDateStr)})
                        </span>
                      )}
                    </span>
                  </>
                ...
                >
                  ...
                  <span className="ml-auto">
                    <Toggle
                      ...
                      onChange={(v) => setValue("repeat_enabled", v)}
                    />
                  </span>
                </button>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() =>
                    setValue("total_slots", Math.max(1, totalSlots - 1))
                  }
                ...
                <span className="w-12 text-center text-lg font-bold text-[#1F2937]">
                  {totalSlots}
                </span>
                <button
                  type="button"
                  onClick={() =>
                    setValue("total_slots", Math.min(100, totalSlots + 1))
                  }
                  className="w-10 h-10 rounded-xl border border-[#E5E7EB] bg-white text-[#374151] text-lg font-semibold hover:border-[#0052CC] hover:text-[#0052CC] transition-colors"
                >
                  +
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                {LEVEL_GROUPS.map((group) => {
                  const allSelected = group.levels.every((l) =>
                    (skillLevels ?? []).includes(l),
                  );
                  const someSelected = group.levels.some((l) =>
                    (skillLevels ?? []).includes(l),
                  );
                  return (
                    <button
          ...
          <div className={cardClass}>
            ...
            <div className="flex items-center justify-between">
              ...
              <Toggle
                ...
                onChange={(v) => setValue("price_enabled", v)}
              />
            </div>
              ...
              <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="relative">
                      <NumericFormat
                        ...
                        value={priceMin ?? ""}
                        onValueChange={(v) =>
                          setValue("price_min", v.floatValue ?? null)
                        }
                      ...
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-[#9CA3AF]">
                        MIN
                      </span>
                    </div>
                    <div className="relative">
                      <NumericFormat
                        ...
                        value={priceMax ?? ""}
                        onValueChange={(v) =>
                          setValue("price_max", v.floatValue ?? null)
                        }
                        className={`w-full text-sm border rounded-xl px-3 py-2.5 pr-12 focus:outline-none focus:border-[#0052CC] focus:ring-2 focus:ring-[#0052CC]/20 transition-colors ${
                          splitEvenly
                            ? "bg-[#F3F4F6] border-[#E5E7EB] text-[#9CA3AF] cursor-not-allowed"
                ...
                <label
                  className={`flex items-start gap-3 p-3 rounded-xl border transition-colors cursor-pointer ${
                    splitEvenly
                      ? "border-[#0052CC] bg-[#EFF6FF]"
                      : "border-[#E5E7EB] bg-[#F9FAFB] hover:border-[#0052CC]"
                  }`}
                >
                  <input
                    type="checkbox"
                  ...
                  <div>
                    <p
                      className={`text-sm font-medium ${splitEvenly ? "text-[#0052CC]" : "text-[#374151]"}`}
                    >
                      Chia đều chi phí sân
                    </p>
                    <p className="text-xs text-[#9CA3AF] mt-0.5">
                      Giá sẽ tự động tính dựa trên tổng tiền sân và số người
                    </p>
                  </div>
                </label>
                ...
                {splitEvenly && (priceMin != null || priceMax != null) && (
                  <div className="flex items-center gap-2 px-3 py-2 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-700 font-medium">
                    ...
                    <span>
                      ...
                      priceMin !== priceMax ? (
                        <>
                          <strong>{priceMin.toLocaleString("vi-VN")}</strong> –{" "}
                        ...
                        <>
                          <strong>
                            {(priceMax ?? priceMin ?? 0).toLocaleString(
                              "vi-VN",
                            )}
                          </strong>{" "}
                          VNĐ
