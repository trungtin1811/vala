...
export default function EditEventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!event || event.host_id !== user?.id) {
    return <div className="max-w-xl mx-auto px-4 py-20 text-center text-[#6B7280]">Không có quyền truy cập.</div>
  }
    ...
    try {
      const { error: eventError } = await supabase
        ...
        .update({
          ...
          event_end_date: computeEndDate(values.event_date, values.event_time, values.event_end_time),
      ...
      await supabase.from('event_skill_requirements').delete().eq('event_id', id)
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      ...
      <h1 className="text-2xl font-bold text-[#1F2937] mb-6">Chỉnh Sửa Vãng Lai</h1>
      {error && <p className="mb-4 text-sm text-[#EF4444] bg-red-50 border border-red-200 rounded-xl px-4 py-3">{error}</p>}
      <EventForm defaultValues={defaultValues} onSubmit={handleSubmit} onCancel={() => router.push(`/events/${id}`)} submitLabel="Lưu thay đổi" loading={submitting} hideRepeat />
    </div>
  )
}
