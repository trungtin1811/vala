...
import { MapPin, Clock, Users, Phone, ChevronLeft, Settings } from 'lucide-react'
...
export default function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleBook() {
    ...
    await bookMutation.mutateAsync({ eventId: id, memberId: user.id, skillLevel: selectedLevel })
  ...
  if (!event) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/events" className="text-[#0052CC] font-medium mt-4 inline-block">← Quay lại</Link>
      </div>
    )
  }

  const availableRequirements = event.skill_requirements?.filter(r => r.slots_booked < r.slots_needed) ?? []
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="p-6 flex flex-col gap-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-[#1F2937]">{formatDate(event.event_date)}</p>
                <p className="text-sm text-[#6B7280]">
                  {formatTimeRange(event.event_time, event.event_end_time ?? null, event.event_date, event.event_end_date ?? null)}
                  {event.event_end_date && event.event_end_date !== event.event_date && (
                    <span className="ml-1 text-amber-600 text-xs">(kết thúc {formatDate(event.event_end_date)})</span>
                  )}
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-[#1F2937]">{event.location}</p>
              </div>
            </div>
          </div>
          ...
          {event.description && (
            <div>
              <h2 className="text-sm font-semibold text-[#1F2937] mb-2">Mô tả</h2>
              <p className="text-sm text-[#6B7280] leading-relaxed whitespace-pre-wrap">{event.description}</p>
            </div>
          )}
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              {event.skill_requirements?.map(req => (
                <div key={req.id} className="flex items-center justify-between p-3 bg-[#F9FAFB] rounded-xl">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-24 h-2 bg-[#E5E7EB] rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${(req.slots_booked / req.slots_needed) * 100}%` }}
                      />
                    </div>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-[#F3F4F6]">
              <Link href={`/profile/${event.host.id}`}>
                {event.host.avatar_url ? (
                  <img src={event.host.avatar_url} alt={event.host.display_name} className="w-10 h-10 rounded-full object-cover" />
                ) : (
              ...
              <div>
                ...
                <Link href={`/profile/${event.host.id}`} className="text-sm font-medium text-[#1F2937] hover:text-[#0052CC]">
          ...
          {bookedHostPhone && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-emerald-700">Đặt chỗ thành công!</p>
                <p className="text-sm text-emerald-600">Liên hệ host: <strong>{bookedHostPhone}</strong></p>
              </div>
            </div>
          )}

          {!isHost && !hasBooked && event.status === 'active' && availableRequirements.length > 0 && (
            <Button
              size="lg"
              className="w-full"
          ...
          {hasBooked && (
            <div className="bg-[#E8F3FF] border border-[#0052CC]/20 rounded-xl p-4 text-center">
              <p className="text-sm font-semibold text-[#0052CC]">✓ Bạn đã đăng ký vãng lai này</p>
              {event.host?.phone && (
                <p className="text-sm text-[#0052CC] mt-1">Liên hệ host: <strong>{event.host.phone}</strong></p>
              )}
            </div>
          )}
        </div>
      ...
      {bookings && bookings.length > 0 && (isHost || hasBooked) && (
        <div className="mt-6 bg-white border border-[#E5E7EB] rounded-2xl p-6">
          <h2 className="text-base font-semibold text-[#1F2937] mb-4">Danh Sách Thành Viên ({bookings.length})</h2>
          <div className="flex flex-col gap-3">
            {bookings.map(booking => (
              <div key={booking.id} className="flex items-center gap-3">
                {booking.member?.avatar_url ? (
                  <img src={booking.member.avatar_url} alt="" className="w-8 h-8 rounded-full object-cover" />
                ) : (
                ...
                <div className="flex-1">
                  ...
                  {isHost && booking.member?.phone && (
                    <p className="text-xs text-[#6B7280]">{booking.member.phone}</p>
                  )}
                </div>
      ...
      <Modal open={bookingModal} onClose={() => setBookingModal(false)} title="Chọn Trình Độ Tham Gia">
        <div className="flex flex-col gap-4">
          <p className="text-sm text-[#6B7280]">Chọn trình độ của bạn để tham gia vãng lai này:</p>
          <div className="flex flex-col gap-2">
          ...
          {bookMutation.isError && (
            <p className="text-xs text-[#EF4444] text-center">Có lỗi xảy ra, vui lòng thử lại.</p>
          )}
        </div>
      </Modal>
    </div>
